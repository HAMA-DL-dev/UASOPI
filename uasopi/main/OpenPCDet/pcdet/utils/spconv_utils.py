...
def replace_feature_adf(mean, var, new_features, new_variance):
    if "replace_feature" in mean.__dir__():
        # spconv 2.x behaviour
        return mean.replace_feature(new_features), var.replace_feature(new_variance) 
    else:
        mean.features = new_features
        var.features = new_variance
        return mean, var