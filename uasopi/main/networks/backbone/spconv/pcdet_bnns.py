...
class PointRCNN(torch.nn.Module):
...
class SECOND(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        # self.classifier = torch.nn.Conv2d(512, out_channels, kernel_size=1)
        self.classifier = adf.Conv2d(512, out_channels, kernel_size=1)
    ...
    def forward(self, data, dropout_rate):
        ...
        outputs = self.model.backbone_3d.forward(
            {"batch_size":batch_size, "voxel_features":features, "voxel_coords":coords}, dropout_rate)
        # outputs = self.model.backbone_3d.forward(
        #     {"batch_size":batch_size, "voxel_features":features, "voxel_coords":coords})
        
        # [TODO] are outputs dictionary type? does outputs have output variance??
        # var = torch.max(outputs['variance'].dense())
        # var = outputs['variance']['var_output'].dense()
        
        outputs = self.model.map_to_bev_module(outputs)
    
        outputs = self.model.backbone_2d(outputs,dropout_rate)
        # outputs = self.model.backbone_2d(outputs) # OpenPCDet에서 사용할 때는 이거 쓰셈 

        var = outputs["variance"]
        outputs = outputs["spatial_features_2d"]
        
        # outputs = self.classifier(outputs)
        # var = self.classifier(var)
        x = outputs, var
        outputs, var = self.classifier(*x)
        ...
        var = var.permute(0, 2, 3, 1).reshape(-1, outputs.shape[1])        
